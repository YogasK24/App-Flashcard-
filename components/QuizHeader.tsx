...
const QuizHeader: React.FC<QuizHeaderProps> = ({ currentCardIndex, totalCards }) => {
  ...
  return (
    <header className="flex justify-between items-center w-full mb-4">
      <h2 className="text-xl font-semibold text-[#E6E1E5]">{quizDeck?.title || 'Kuis'}</h2>
      <div className="flex items-center space-x-4">
        <span className="text-[#C8C5CA]">{`${currentCardIndex} / ${totalCards}`}</span>
        <button onClick={endQuiz} aria-label="Tutup kuis">
          <Icon name="trash" className="w-6 h-6 text-[#E6E1E5]" />
        </button>
      </div>
    </header>
  );
